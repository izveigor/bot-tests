...
class Validator:
    ...
    def __init__(
        ...
        Возвращает: None
        """
        is_command_right(command, errors)
        is_name_right(name, errors)
        is_description_right(description, errors)
        are_questions_right(questions, errors)
        is_result_explanation_right(result_explanation, len(questions), errors)


def is_command_right(command: Any, errors: list[Union[str, int]]) -> None:
    ...
    if CommandsTestTree().search(Node(Test(command, "", None, [], None))) is not None:
        raise BotParseException(errors, "Тест с такой командой уже существует.")


def is_name_right(name: Any, errors: list[Union[str, int]]) -> None:
    ...
    if not isinstance(name, str) or not name:
        raise BotParseException(errors, "Название теста должно быть непустой строкой.")
...
def is_description_right(description: Any, errors: list[Union[str, int]]) -> None:
...
def are_questions_right(questions: Any, errors: list[Union[str, int]]) -> None:
    ...
    if not isinstance(questions, list) or len(questions) == 0:
        raise BotParseException(errors, "Вопросы теста должны быть непустым списком.")

    for number, question in enumerate(questions, start=1):
        if not isinstance(question, dict):
            raise BotParseException(errors, f"{number}-й вопрос не является словарем.")
...
def is_result_explanation_right(
...
class _QuestionValidator:
    def __init__(
        ...
        is_question_body_right(body, errors)
        is_widget_right(widget, errors)
        is_answer_right(answer, widget, errors)
        is_answer_explanation_right(answer_description, errors)


def is_question_body_right(body: Any, errors: list[Union[str, int]]) -> None:
...
def is_widget_right(widget: Any, errors: list[Union[str, int]]) -> None:
...
def is_answer_right(answer: Any, widget: Any, errors: list[Union[str, int]]) -> None:
    ...
    if widget_type == "input":
        ...
        if len(set(answer)) != len(answer):
            duplicates = {checked for checked in answer if answer.count(checked) > 1}
            raise BotParseException(
                errors,
...
def is_answer_explanation_right(
...
class _WidgetValidator:
    def __init__(
        ...
        is_type_right(widget_type, errors)
        is_widget_body_right(widget_body, widget_type, errors)


def is_type_right(widget_type: Any, errors: list[Union[str, int]]) -> None:
